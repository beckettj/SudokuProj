...
// The key MUST be directly in the URL for Pollinations to recognize your paid account
  const imageUrl = `https://gen.pollinations.ai/image/${encodeURIComponent(prompt)}?width=200&height=200&nologo=true&seed=${seed}&model=flux&key=${process.env.POLLINATIONS_API_KEY}`;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      const response = await fetch(imageUrl, {
        headers: { 'User-Agent': 'ThemeSudoku/1.0' }, // Removed the useless auth header
