...
const AITheme = (() => {
  ...
  async function fetchImages(theme, onProgress) {
    ...
    const res = await postWithRetry('https://sudokuproj-production.up.railway.app.up.railway.app/api/get-theme', { theme });
    ...
    const imageData = terms.slice(0, 9).map((term, i) => ({
      label: term,
      url: `https://sudokuproj-production.up.railway.app.up.railway.app/api/image/${encodeURIComponent(term)}?seed=${i}`,
      loaded: false,
      failed: false,
      isClassic: false,
    }));
